...
seed_file = processed / "phase47_mechanism_seed_stage5_verified.csv"
...
expected_stage5_seed = {
    "aly688-sr": "VERIFIED",
    "amg 171": "VERIFIED",
    "at673 high dose": "VERIFIED_ALIAS",
    "at673 low dose": "VERIFIED_ALIAS",
    "azd1043": "RESEARCH_REQUIRED",
    "azelaprag bge-105": "VERIFIED_ALIAS",
    "bge-102": "VERIFIED",
    "bi 1356225": "VERIFIED",
}
...
for therapy_name, expected_status in expected_stage5_seed.items():
...
if len(seed) != 156:
    seed_errors.append(
        f"expected exactly 156 seed rows, found {len(seed)}"
    )

if seed_errors:
    raise ValueError(
        "Wrong or stale Stage-5 mechanism seed loaded:\n- "
        + "\n- ".join(seed_errors)
    )

print("Stage-5 seed sanity check: PASSED")

print("=== Phase 4.7: mechanism / target / modality stage 5 ===")
