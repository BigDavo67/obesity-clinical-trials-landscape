...
seed_file = processed / "phase47_mechanism_seed_stage9_verified.csv"
...
expected_stage9_seed = {
    "exenatide once weekly eqw": "VERIFIED_ALIAS",
    "exendin-4 fc fusion protein jy09": "VERIFIED",
    "g3215": "VERIFIED",
    "gly-200": "VERIFIED",
    "gma106": "VERIFIED",
    "gub-ucn2": "VERIFIED",
...
for therapy_name, expected_status in expected_stage9_seed.items():
...
if len(seed) != 233:
    seed_errors.append(
        f"expected exactly 233 seed rows, found {len(seed)}"
    )

if seed_errors:
    raise ValueError(
        "Wrong or stale Stage-9 mechanism seed loaded:\n- "
        + "\n- ".join(seed_errors)
    )

print("Stage-9 seed sanity check: PASSED")

print("=== Phase 4.7: mechanism / target / modality stage 9 ===")
