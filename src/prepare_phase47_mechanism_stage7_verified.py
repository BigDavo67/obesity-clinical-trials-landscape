...
seed_file = processed / "phase47_mechanism_seed_stage7_verified.csv"
...
expected_stage7_seed = {
    "ck-0045": "VERIFIED",
    "cms-d008": "VERIFIED",
    "crb-913": "VERIFIED",
    "ct-868": "VERIFIED",
    "ct-996": "VERIFIED_ALIAS",
    "cx11 vct220": "VERIFIED_ALIAS",
    "da-1726": "VERIFIED",
    "dccr diazoxide choline": "VERIFIED_ALIAS",
}
...
for therapy_name, expected_status in expected_stage7_seed.items():
...
if len(seed) != 195:
    seed_errors.append(
        f"expected exactly 195 seed rows, found {len(seed)}"
    )

if seed_errors:
    raise ValueError(
        "Wrong or stale Stage-7 mechanism seed loaded:\n- "
        + "\n- ".join(seed_errors)
    )

print("Stage-7 seed sanity check: PASSED")

print("=== Phase 4.7: mechanism / target / modality stage 7 ===")
